...
const Modal = ({ isOpen, onClose, onSave, initialData, isEdit }) => {
  ...
  const handleImageUpload = async (e, index) => {
    ...
    setIsImageUploading((prev) => ({ ...prev, [index]: true }));

    const updatedEntries = [...formData.diseaseEntries];
    const imageUrls = [...(updatedEntries[index].images || [])];

    for (const file of files) {
      const maxSize = 5 * 1024 * 1024; // 5MB
      if (file.size > maxSize) {
        alert('File size exceeds 5MB limit.');
        setIsImageUploading((prev) => ({ ...prev, [index]: false }));
        return;
      }
    ...
    updatedEntries[index].images = imageUrls;
    setFormData({ ...formData, diseaseEntries: updatedEntries });
    setIsImageUploading((prev) => ({ ...prev, [index]: false }));
  };

  const handleDeleteImage = (entryIndex, imgIndex) => {
    const updatedEntries = [...formData.diseaseEntries];
    updatedEntries[entryIndex].images = updatedEntries[entryIndex].images.filter((_, i) => i !== imgIndex);
    setFormData({ ...formData, diseaseEntries: updatedEntries });
  };
  ...
  const addEntry = () => {
    setFormData({
      ...
      diseaseEntries: [
        ...
        { diseaseName: '', diseaseSeverity: '', remarks: '', diseasesIncident: '', diseasesIncidentRemarks: '', images: [] },
      ],
    });
  };
      ...
      <div className="bg-white px-4 sm:px-6 py-6 sm:py-8 rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="mt-4 sm:mt-6">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-base sm:text-lg font-semibold">Disease Entries</h3>
            <button
              onClick={addEntry}
              className="p-2 text-2xl text-black rounded hover:bg-gray-100"
              type="button"
            >
              <IoIosAdd />
            </button>
          ...
          {formData.diseaseEntries.map((entry, index) => (
            <div key={index} className="border p-3 sm:p-4 rounded mb-3">
              <div className="flex justify-between items-center mb-2">
                <h4 className="text-sm font-semibold">Disease_{index + 1}</h4>
                <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                ...
                <div>
                  <select
                    ...
                    onChange={(e) => handleChange(e, index)}
                    className={`p-2 border w-full rounded text-sm ${errors[`diseaseSeverity_${index}`] ? 'border-red-500' : ''}`}
                  >
                  ...
                  {errors[`diseaseSeverity_${index}`] && (
                    <p className="text-red-500 text-xs mt-1">{errors[`diseaseSeverity_${index}`]}</p>
                  )}
                </div>
                ...
                <input
                  ...
                  readOnly
                  placeholder="Remarks"
                  className="p-2 border bg-gray-100 rounded text-sm"
                />
                ...
                <div>
                  <input
                    ...
                    className={`p-2 border w-full rounded text-sm ${errors[`diseasesIncident_${index}`] ? 'border-red-500' : ''}`}
                  />
                  {errors[`diseasesIncident_${index}`] && (
                    <p className="text-red-500 text-xs mt-1">{errors[`diseasesIncident_${index}`]}</p>
                  )}
                </div>
              ...
              <div className="mt-3">
                <label htmlFor={`images_${index}`} className="block text-sm font-medium text-gray-700 mb-1">Upload Images</label>
                <input
                  id={`images_${index}`}
                  type="file"
                  accept="image/*"
                  ...
                  disabled={isImageUploading[index]}
                />
                {isImageUploading[index] && <p className="text-gray-500 text-xs mt-1">Uploading images...</p>}
